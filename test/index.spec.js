...
describe('Dagger', () => {
  describe('Initialization', () => {
    it('should throw error for no url', () => {
      expect(() => new Dagger()).to.throw('`url` (string) is required as first argument');
    });
  ...
  describe('Subscribe once', () => {
    ...
    it('should add complex subscription once', () => {
      ...
      expect(dagger.getSubscriptions()).to.deep.equal(['latest:log/0x123456/filter/#']);
  ...
  describe('Match', () => {
    ...
    it('should match simple topics', () => {
      ...
      dagger.on('latest:block.number', noop);
      expect(dagger.getMatchingTopics('latest:block.number')).to.deep.equal(['latest:block.number']);
    });

    it('should match wildcard + topics', () => {
      dagger.on('latest:addr/+/tx', noop);
      expect(dagger.getMatchingTopics('latest:addr/0x123456/tx')).to.deep.equal(['latest:addr/+/tx']);
      expect(dagger.getMatchingTopics('latest:addr/0xabcdef/tx')).to.deep.equal(['latest:addr/+/tx']);
      expect(dagger.getMatchingTopics('latest:addr/tx')).to.deep.equal([]);

      dagger.on('latest:addr/+/tx/+', noop);
      expect(dagger.getMatchingTopics('latest:addr/0x123456/tx/in')).to.deep.equal(['latest:addr/+/tx/+']);
      expect(dagger.getMatchingTopics('latest:addr/0xabcdef/tx/in')).to.deep.equal(['latest:addr/+/tx/+']);
      expect(dagger.getMatchingTopics('latest:addr/tx/in')).to.deep.equal([]);
      expect(dagger.getMatchingTopics('latest:addr/0x123456/tx/out')).to.deep.equal(['latest:addr/+/tx/+']);
      expect(dagger.getMatchingTopics('latest:addr/0xabcdef/tx/out')).to.deep.equal(['latest:addr/+/tx/+']);
      expect(dagger.getMatchingTopics('latest:addr/tx/in')).to.deep.equal([]);
    });

    it('should match wildcard # topics', () => {
      dagger.on('latest:log/0x1234567/filter/#', noop);
      expect(dagger.getMatchingTopics('latest:log/0x1234567/filter'))
        .to.deep.equal(['latest:log/0x1234567/filter/#']);
      expect(dagger.getMatchingTopics('latest:log/0x1234567/filter/0x9876543'))
        .to.deep.equal(['latest:log/0x1234567/filter/#']);
      expect(dagger.getMatchingTopics('latest:log/0x1234567/filter/0x9876543/0xface'))
        .to.deep.equal(['latest:log/0x1234567/filter/#']);
      expect(dagger.getMatchingTopics('latest:log/0x1234567/filter/0x9876543/0xdeadbeef'))
        .to.deep.equal(['latest:log/0x1234567/filter/#']);
      expect(dagger.getMatchingTopics('latest:log/0xabcdefg/filter')).to.deep.equal([]);
    });
  });
});
