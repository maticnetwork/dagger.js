...
const ApprovalABI = '0x8c5be1e5ebec7d5bd14f71427d1e84f3dd0314c0f7b2291e5b200ac8c7c3b925';
...
describe('Contract', () => {
  describe('Initialization', () => {
    ...
    it('should set route', () => {
      ...
      expect(subscription.route).to.be.equal(`latest:log/${BATAddress}/filter/${TransferABI}/+/+`);

      subscription = contract.events.Transfer({ filter: { _to: GNTAddress } });
      expect(subscription.route).to.be.equal(`latest:log/${BATAddress}/filter/${TransferABI}/+/${web3.utils.leftPad(GNTAddress, 64)}`);

      // Approval method
      subscription = contract.events.Approval();
      expect(subscription.route).to.be.equal(`latest:log/${BATAddress}/filter/${ApprovalABI}/+/+`);

      subscription = contract.events.Approval({ filter: { _spender: GNTAddress } });
      expect(subscription.route).to.be.equal(`latest:log/${BATAddress}/filter/${ApprovalABI}/+/${web3.utils.leftPad(GNTAddress, 64)}`);

      subscription = contract.events.Approval({
        filter: { _spender: GNTAddress, _owner: BATAddress },
      });
      expect(subscription.route).to.be.equal(`latest:log/${BATAddress}/filter/${ApprovalABI}/${web3.utils.leftPad(BATAddress, 64)}/${web3.utils.leftPad(GNTAddress, 64)}`);
    });

    it('should set route for all events', () => {
      // allEvents method
      const subscription = contract.events.allEvents();
      expect(subscription.route).to.be.equal(`latest:log/${BATAddress}/filter/#`);
    });

    it('should set room', () => {
      let subscription = contract.events.Transfer({ room: 'confirmed' });
      expect(subscription.route).to.be.equal(`confirmed:log/${BATAddress}/filter/${TransferABI}/+/+`);

      subscription = contract.events.Approval({ room: 'confirmed' });
      expect(subscription.route).to.be.equal(`confirmed:log/${BATAddress}/filter/${ApprovalABI}/+/+`);
    ...
    it('should set room for all events', () => {
      ...
      expect(subscription.route).to.be.equal(`confirmed:log/${BATAddress}/filter/#`);
    });
  });
  ...
  describe('Subscribe and unsubscribe', () => {
    ...
    it('should watch', () => {
      ...
      expect(dagger.getSubscriptions()).to.deep.equal([`latest:log/${BATAddress}/filter/${TransferABI}/+/+`]);
    ...
    it('should watch once', () => {
      ...
      // emit
      emitMatch(`latest:log/${BATAddress}/filter/${TransferABI}/0x1/0x2`, TransferLog);
      expect(dagger.getSubscriptions()).to.deep.equal([]);
    });
  });
});
